...
async function main() {
    ...
    for (const plan of plans) {
        const created = await prisma.plan.upsert({
            where: { id: `00000000-0000-0000-0000-${plan.name.toLowerCase().padStart(12, '0')}` },
            update: { ...plan, features: plan.features },
            create: {
                id: `00000000-0000-0000-0000-${plan.name.toLowerCase().padStart(12, '0')}`,
                ...plan,
    ...
    // Assign role via raw — user_roles needs tenantId but super_admin has none
    // We use a sentinel UUID for platform-level roles

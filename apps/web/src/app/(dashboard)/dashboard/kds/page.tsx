...
export default function KdsPage() {
  ...
  const activeModules = useAuthStore((s) => s.user?.activeModules);
  ...
  // `/v1/orders` est un endpoint partagé non gaté par module (utilisé par tous
  // les plans pour la page Commandes) : le contrôle d'accès à cet écran KDS
  // dédié se fait donc côté client, comme la visibilité de son lien de nav.
  const kdsAllowed = !activeModules || activeModules.includes('kds');

  useEffect(() => {
    if (!kdsAllowed) router.replace('/dashboard/orders');
  }, [kdsAllowed, router]);
